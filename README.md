...
init.md
  ...
  └── procedures/GIT_SYNC.md               ← repo sync check
  └── Ask user: ad-hoc / new / resume / complete
...
├── .gitignore
├── START_HERE.md                 master loading order and permissions
│
├── agent/
│   └── AGENT.md                  agent identity, session rules, capability self-check
│
...
context-glue uses a single `agent/AGENT.md` that works with any AI agent. At session start, the agent runs a capability self-check and tells you upfront what works in its environment (file access, terminal, credentials). No agent-specific configuration needed.
