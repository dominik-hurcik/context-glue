...
5. Scan all three files for findings worth promoting to shared knowledge. Look for:
   ...
   Use `context-glue/knowledge/INDEX.md` to identify the right room for each finding.
...
7. For each finding worth promoting, present it before writing:
...
9. Run `context-glue/procedures/KNOWLEDGE_PR_CHECKLIST.md` — the "For the agent" section — against every file you modified. Report any flags (contradictions, duplication, wrong room, size concern) before telling the user to push.

10. Update line counts in `context-glue/knowledge/INDEX.md` for every file that changed. Update `## Last updated:` in INDEX.md if you changed it.
